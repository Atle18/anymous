...
from dpgnn.model import DPGNN, train
...
''' Configuration '''
flags.DEFINE_string('data_file', 'data/cora_ml.npz', 'Path to the .npz data file')
...
flags.DEFINE_integer('topk', 16, 'Number of PPR neighbors for each node')
...
flags.DEFINE_float('sigma_ista', 0.01, 'Gaussian noise standard deviation for DP-ISTA')
flags.DEFINE_float('clip_bound_ista', 0.01, 'Gradient clip bound for DP-ISTA')
flags.DEFINE_float('EM_eps', 0.08, 'Privacy budget epsilon for EM')
flags.DEFINE_float('report_val_eps', 0.5, 'Privacy budget epsilon for reporting topk noise values for each node')
...
flags.DEFINE_float('sigma_sgd', 0.89, 'Gaussian noise standard deviation for DP-SGD')
...
def main(unused_argv):
    ...
    if FLAGS.dp_sgd:
        ...
        privacy_record_sgd = f'''DP budget for DP-SGD: epsilon={eps_sgd:.4f}, delat={delta}'''
    ...
    model = DPGNN(d, nc, FLAGS.hidden_size, FLAGS.nlayers, FLAGS.lr, FLAGS.weight_decay, FLAGS.dropout,
                  FLAGS.clip_bound_sgd, FLAGS.sigma_sgd, FLAGS.microbatches, dp_sgd=FLAGS.dp_sgd,
